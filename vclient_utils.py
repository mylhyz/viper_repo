...
def safe_rename(old, new):
    ...
    for i in range(retries):
        try:
            ...
            # retry
            logging.debug(
                "Renaming failed from %s to %s. Retrying ..." % (old, new))
            time.sleep(0.1)
...
class Annotated(Wrapper):
    ...
    def __init__(self, wrapped, include_zero=False):
        ...
        self._wrapped_write = getattr(
            self._wrapped, 'buffer', self._wrapped).write
...
def CheckCallAndFilter(args, print_stdout=False, filter_fn=None,
    ...
    if print_stdout:
        ...
        def stdout_write(_): return None
    ...
    def _run_one_task(self, task_item, args, kwargs):
        if self.jobs > 1:
            ...
            try:
                ...
                print('[%s] Started.' %
                      Elapsed(task_item.start), file=task_item.outbuf)
                ...
                if self.progress:
                    self.progress.update(1, ', '.join(
                        t.item.name for t in self.running))
            except KeyboardInterrupt:
                print(
                    self.format_task_output(task_item, 'interrupted'), file=sys.stderr)
                raise
            except Exception:
                print(self.format_task_output(
                    task_item, 'ERROR'), file=sys.stderr)
                raise
    ...
    class _Worker(threading.Thread):
        ...
        def run(self):
            ...
            try:
                ...
                print('[%s] Started.' %
                      Elapsed(self.item.start), file=self.item.outbuf)
                ...
                logging.info(
                    'Caught KeyboardInterrupt in thread %s', self.item.name)
...
def RunEditor(content, git, git_editor=None):
    ...
    file_handle, filename = tempfile.mkstemp(
        text=True, prefix='cl_description')
...
def ParseCodereviewSettingsContent(content):
    ...
    try:
        keyvals = dict([x.strip() for x in l.split(':', 1)]
                       for l in lines if l)
    except ValueError:
        raise Error(
...
def Unzip(file_path, target_path, folder_prefix='dist'):
    ...
    for name in zip_file.namelist():
        ...
        if name.startswith(folder_prefix):
            dist_file = os.path.join(target_path, name[len(folder_prefix):])
